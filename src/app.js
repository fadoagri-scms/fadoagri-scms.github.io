const titles = {
  ...
  (function(){
      ...
      groups.forEach(function(items){
        ...
        items.forEach(function(r, itemIdx){
          ...
          boxes.forEach(function(box, subIdx){
            ...
            // Ở các dòng quy cách phụ (subIdx>0), các ô rowspan (Hao hụt/Bắt
            // đầu/Kết thúc/Tổng/Thao tác) không lặp lại nên soLuongThungTd vô
            // tình thành ô cuối cùng trong <tr> đó — CSS
            // "td:last-child{text-align:right}" (dành riêng cho cột Thao
            // tác) sẽ bắt nhầm, làm số liệu lúc lệch trái lúc lệch phải
    ...
    function createBoxRow(quyCach, soLuongThung){
    ...
    function resetBoxRows(boxes){
      ...
      if(boxes && boxes.length){
        boxes.forEach(function(b){ createBoxRow(b.quy_cach, b.so_luong_thung); });
      } else {
        createBoxRow();
      }
    }
    ...
    function readBoxRows(){
      ...
      return Array.from(boxesListEl.querySelectorAll('.box-row')).map(function(row){
        ...
        return { quyCach: parseQty(inputs[0].value), soLuongThung: parseQty(inputs[1].value) };
        ...
        if(factoryBatchId){
          ...
          if(boxRows.length){
            const { error: insErr } = await sb.from('factory_batch_boxes').insert(boxRows.map(function(r){
              return { factory_batch_id: factoryBatchId, quy_cach: r.quyCach, so_luong_thung: r.soLuongThung };
            }));
            if(insErr) throw insErr;
          }
        }
  ...
  (function(){
    ...
    async function refreshInventoryRows(){
      try{
        ...
        // phẩm/sản phẩm riêng, rồi tách tiếp theo TỪNG Quy cách đã đóng gói
        // (boxesByQuyCach) vì "Đã xuất" giờ theo dõi riêng theo từng quy cách.
        const varietyMap = {};
        function ensureVariety(batch, variety){
          const key = varietyKey(batch, variety);
          if(!varietyMap[key]) varietyMap[key] = { batch: batch, variety: variety, sanPham: null, boxesByQuyCach: {} };
          return varietyMap[key];
        }
        function ensureBox(v, quyCach){
          const qKey = quyCachKeyOf(quyCach);
          if(!v.boxesByQuyCach[qKey]) v.boxesByQuyCach[qKey] = { quyCach: quyCach, produced: 0 };
          return v.boxesByQuyCach[qKey];
        }
        (rawRes.data || []).forEach(function(r){
          ...
          (fb.factory_batch_boxes || []).forEach(function(box){
            ensureBox(v, box.quy_cach).produced += (Number(box.so_luong_thung) || 0);
          });
        });
        ...
        (stockRes.data || []).forEach(function(s){
          ...
          stockByFullKey[varietyKey(s.batch, variety) + '::' + quyCachKeyOf(s.quy_cach)] = s;
          // Đã có bản ghi xuất cho quy cách này thì vẫn phải hiện ra dù
          // Xưởng sản xuất hiện không còn đợt nào khớp đúng quy cách đó
          // (không được để mất dữ liệu xuất đã nhập).
          ensureBox(v, s.quy_cach);
        });
        ...
        Object.values(varietyMap).forEach(function(v){
          const qKeys = Object.keys(v.boxesByQuyCach);
          const quyCachEntries = (qKeys.length ? qKeys : [quyCachKeyOf(null)]).map(function(qKey){
            const box = v.boxesByQuyCach[qKey] || { quyCach: null, produced: 0 };
            const stock = stockByFullKey[varietyKey(v.batch, v.variety) + '::' + qKey];
    ...
    function closeModal(){ inventoryOverlay.classList.remove('active'); inventoryForm.reset(); editingBatch = null; editingVariety = null; editingQuyCach = null; }
    ...
    function openEditModal(tr){
      ...
      if(inventoryModalBatchInfo){
        ...
        inventoryModalBatchInfo.textContent = 'Lô hàng: ' + tr.dataset.batch + varietyLabel + quyCachLabel +
    ...
    async function deleteStockRow(tr){
      ...
      if(!confirm('Xóa bản ghi xuất hàng của lô "' + tr.dataset.batch + '"' + varietyLabel + quyCachLabel + '? Hành động không thể hoàn tác.')) return;
      try{
    ...
    inventoryForm.addEventListener('submit', async function(e){
      ...
      try{
        const { error } = await sb.from('factory_finished_stock').upsert(payload, { onConflict: 'batch,chung_loai,quy_cach' });
        if(error) throw error;
        await refreshInventoryRows();
        closeModal();
