const titles = {
  ...
  (function(){
    ...
    async function refillTraceFromSystem(silent){
      ...
      try{
        ...
        let gotAny = false;
        if(items.length){ tracePackingTextInput.value = formatPackingTextFromBoxes(items); gotAny = true; }
        if(regionSuggestion){ traceRegionInput.value = regionSuggestion; gotAny = true; }
        if(packedDateSuggestion){ tracePackedDateInput.value = packedDateSuggestion; gotAny = true; }
    ...
    if(traceRefillBtn) traceRefillBtn.addEventListener('click', function(){ refillTraceFromSystem(false); });
    ...
    async function openTraceModal(batchCode){
      ...
      try{
        const { data, error } = await sb.from('batch_info').select('trace_region,trace_packed_date,trace_packing_text,trace_enabled,public_trace_code').eq('batch', batchCode).maybeSingle();
        if(error) throw error;
        const bi = data || {};
        ...
        if(!bi.trace_region && !bi.trace_packed_date && !bi.trace_packing_text) refillTraceFromSystem(true);
